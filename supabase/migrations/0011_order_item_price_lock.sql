-- Agregar un parametro nuevo (aunque tenga default) cambia la firma de la
-- funcion: create or replace por si solo dejaria la version vieja de 5
-- argumentos como un overload separado en vez de reemplazarla. Hay que
-- dropearla primero.
drop function if exists public.update_order(uuid, uuid, uuid, date, jsonb);

-- Hasta ahora, guardar cualquier cambio en un pedido (aunque fuera solo la
-- cantidad de un item) resincronizaba en silencio TODOS los precios al
-- precio actual del catalogo, porque update_order siempre usaba
-- products.price_cents. Eso significaba que si se resubia la lista de
-- precios con valores nuevos, el pedido cambiaba de precio sin que nadie lo
-- viera ni lo confirmara. Ahora el default es mantener el precio original
-- con el que se cargo el pedido, y solo se resincroniza al precio actual
-- para los productos que el usuario confirmo explicitamente (desde la
-- pantalla de edicion, boton "Actualizar precio" por item) via
-- p_sync_price_product_ids. El servidor siempre saca los dos numeros (viejo
-- y nuevo) de sus propias tablas — el cliente nunca manda un precio, solo
-- elige a que productos aplicarles el precio actual.
...
revoke all on function public.update_order(uuid, uuid, uuid, date, jsonb, uuid[]) from public;
grant execute on function public.update_order(uuid, uuid, uuid, date, jsonb, uuid[]) to authenticated;

-- Postgres no permite cambiar la firma de una funcion existente sin dropear
-- la vieja cuando se agrega un parametro por posicion en vez de al final
-- con default (esto ya lo cumple: el nuevo parametro tiene default, asi que
-- create or replace alcanza y no hace falta drop). Se deja explicito el
-- grant de arriba por las dudas de que create or replace resetee permisos.
