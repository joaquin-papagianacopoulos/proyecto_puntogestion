-- Por default la nota es solo un recordatorio interno (se borra al
-- facturar, como se armo en 0012). Pero a veces la nota tiene que llegar a
-- la boleta ("entregar en el local de atras", "cliente pidio factura A")
-- — show_note_on_invoice, tildado desde el pedido, hace que mark_order_invoiced
-- la deje intacta en vez de limpiarla, y que aparezca impresa en el PDF.
alter table public.orders add column show_note_on_invoice boolean not null default false;
...
create or replace function public.create_order(
  ...
  p_show_note_on_invoice boolean default false
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.orders (organization_id, client_id, vendedor_membership_id, total_cents, note, show_note_on_invoice)
  values (p_organization_id, p_client_id, v_membership_id, v_total_cents, nullif(trim(p_note), ''), p_show_note_on_invoice)
...
revoke all on function public.create_order(uuid, uuid, jsonb, text, boolean) from public;
grant execute on function public.create_order(uuid, uuid, jsonb, text, boolean) to authenticated;

drop function if exists public.update_order(uuid, uuid, uuid, date, jsonb, uuid[], text);
...
begin
  ...
  set client_id = p_client_id,
      ...
      note = v_new_note,
      show_note_on_invoice = p_show_note_on_invoice,
      updated_at = now()
  where id = p_order_id;
...
revoke all on function public.update_order(uuid, uuid, uuid, date, jsonb, uuid[], text, boolean) from public;
grant execute on function public.update_order(uuid, uuid, uuid, date, jsonb, uuid[], text, boolean) to authenticated;
...
begin
  ...
  -- La nota se borra al facturar salvo que se haya marcado explicitamente
  -- "mantener en la factura" (show_note_on_invoice) al cargarla/editarla.
  update public.orders
  set status = 'facturado',
      invoiced_at = now(),
      updated_at = now(),
      note = case when show_note_on_invoice then note else null end
  where id = p_order_id
    and organization_id = p_organization_id
