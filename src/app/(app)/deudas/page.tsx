...
export default async function DeudasPage({
  ...
  if (direction !== "todas") {
    query = query.eq("direction", direction as "nos_deben" | "debemos");
  }
    ...
    <>
      ...
      <div className="grid gap-3 pb-24">
        {(debts ?? []).map((debt) => {
          ...
          const balance = debt.amount_cents - paid;
        ...
        {(debts ?? []).length === 0 ? <p className="text-sm text-neutral-500">No hay deudas cargadas.</p> : null}
      </div>
